...
sys_exofork(void)
{
  ...
  // Allocating a new environment
	struct Env *e;
	int r = env_alloc(&e, curenv ? curenv->env_id : 0); // We can set the parent env_id 0 if there is no curenv

  // Return < 0 on error
  if (r < 0)
    return r;
  
	e->env_status = ENV_NOT_RUNNABLE; // Set new env status as ENV_NOT_RUNNABLE
  e->env_tf = curenv->env_tf; // Copy registers (trapframe stores registers)
	e->env_tf.tf_regs.reg_eax = 0; // Set child return value as 0
...
sys_env_set_status(envid_t envid, int status)
{
  ...
  // If status is not either of ENV_RUNNABLE or ENV_NOT_RUNNABLE then return -E_INVAL
	if (status != ENV_NOT_RUNNABLE && status != ENV_RUNNABLE) 
    return -E_INVAL;

	struct Env *e; 
   
  // Using envid2env to check if envid is valid or not
  // Return < 0 on error
	if (envid2env(envid, &e, 1) < 0) 
    return -E_BAD_ENV;
    
	e->env_status = status; // Set envid's env_status to status
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
  ...
  // Return -E_BAD_ENV if environment envid doesn't currently exist
	...
	e->env_pgfault_upcall = func; // Set the page fault upcall for 'envid'
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
  ...
  // Checking if va is page aligned 
  if ((uintptr_t) va & 0xfff) 
      return -E_INVAL;  
  
  // Checking if perm only includes bits in PTE_SYSCALL
  if ((perm & 0xfff) & (~PTE_SYSCALL))
    return -E_INVAL;
  
	struct PageInfo *pp = page_alloc(ALLOC_ZERO); // Get a free physical page
 
  // Return -E_NO_MEM if there are no free physical pages
	if (!pp) 
    return -E_NO_MEM;
    
	pp->pp_ref++;
	r = page_insert(e->env_pgdir, pp, va, perm); // Map it
 
  // Free the page if page_insert() failed
	if (r) {
		page_free(pp);
    return r;
  }
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
  ...
  // Return if srcenvid and/or dstenvid doesn't currently exist, or the caller doesn't have permission to change one of them.
	if (envid2env(srcenvid, &srcenv, 1) < 0 || envid2env(dstenvid, &dstenv, 1) < 0) 
    return -E_BAD_ENV;
  
  // Return -E_INVAL if srcva >= UTOP or srcva is not page-aligned, or dstva >= UTOP or dstva is not page-aligned.
	if ((srcva >= (void*) UTOP || (uintptr_t) srcva & 0xfff) || (dstva >= (void*) UTOP || (uintptr_t) dstva & 0xfff)) 
		return -E_INVAL;

  // Get the struct PageInfo *pp of srcva
	pte_t *pte;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
  ...
  if (srcva < (void *) UTOP && ((perm & 0xfff) & (~PTE_SYSCALL)))
    return -E_INVAL;  
  ...
  if (e->env_ipc_dstva < (void *) UTOP && page_insert(e->env_pgdir, pp, e->env_ipc_dstva, perm))
      return -E_NO_MEM;
  ...
  e->env_ipc_from = curenv->env_id; // env_ipc_from is set to the sending envid;
	...
	e->env_status = ENV_RUNNABLE; // Set status as ENV_RUNNABLE
...
sys_ipc_recv(void *dstva)
{
  ...
  // If 'dstva' is < UTOP, then you are willing to receive a page of data.
	...
	curenv->env_ipc_recving = 1; // Indicate the env is waiting for a message
	curenv->env_status = ENV_NOT_RUNNABLE; // Set env_status = NOT_RUNNABLE
  curenv->env_tf.tf_regs.reg_eax = 0; // Set tf_regs.regs_eax = 0. It will return 0 if recv succeeds
	sched_yield();
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
    case SYS_cputs: 
      sys_cputs((const char *)a1, (size_t) a2); // dispatch system calls by eax and argument values
      return 0;
    case SYS_cgetc: 
      return sys_cgetc();
    case SYS_getenvid:
    ...
    case SYS_ipc_recv:
  	...
  	default: // return -E_INVAL if system call number is invlaid
  		return -E_INVAL;
	}
}
