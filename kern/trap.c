...
trap_init(void)
{
  ...
  SETGATE(idt[T_BRKPT], 0, GD_KT, &t_brkpt, 3); // T_BRKPT must be available to Ring 3
  ...
  SETGATE(idt[T_SYSCALL], 0, GD_KT, &t_syscall, 3); // T_SYSCALL must be available to Ring 3
...
trap_dispatch(struct Trapframe *tf)
{
  ...
  switch (tf->tf_trapno) { 
    case T_PGFLT: // dispatch page_fault
      return page_fault_handler(tf);
    case T_BRKPT: // dispatch breakpoint
      return monitor(tf);
    case T_SYSCALL: // dispatch system call trap
      // read all register values from Trapframe
      // invoke syscall
...
page_fault_handler(struct Trapframe *tf)
{
  ...
  if (curenv->env_pgfault_upcall) {
    ...
    // Put UTrapframe in UXSTACK
    if (ROUNDUP(tf->tf_esp, PGSIZE) == UXSTACKTOP)
      utf_esp = tf->tf_esp - 4 - sizeof(struct UTrapframe); // If this is a nested exception
    else
      utf_esp = UXSTACKTOP - sizeof(struct UTrapframe); // If this is a new exception

		user_mem_assert(curenv, (void*) utf_esp, 1, PTE_W);
		
    // Create a trapframe
    struct UTrapframe *utf = (struct UTrapframe *) utf_esp;
    ...
    // Run the function curenv->env_pgfault_upcall() in Ring 3
    curenv->env_tf.tf_esp = utf_esp;
